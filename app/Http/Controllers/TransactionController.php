...
class TransactionController extends Controller
{
    ...
    public function earnings() {
        ...
        if($user->id==1){
            ...
            $cleared = Transaction::with(['commission_from','user'])->whereIn('trans_type', ['2','5'])->where('amount','>',0)->where('cleared',1)->sum('amount');
            ...
            // $earnings = UserCommission::where('user_id', $user->id)->get();
            $cleared = Transaction::with(['commission_from'])->where('user_id', $user->id)->where('amount','>',0)->whereIn('trans_type', ['2','5'])->where('cleared',1)->sum('amount');
...
// 🔸 Referral Bonus Program 2023🔸 - STILL ONGOING!
    ...
    protected function findMatchV2($parentid, $newmemberid, $comm_rate){
        $user = Auth::user();
        $members = User::where('parent_referral',$parentid)->where("status",1)->get(['id']);
        $parent = User::where('id',$parentid)->where("status",1)->first();
        
        $match_trans=Transaction::whereIn('commission_from',$members)->where('user_id', $parentid)->where('amount', $comm_rate)->count();
